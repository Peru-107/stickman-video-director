...
### 把任何想法，变成一支真正“动起来”的 30 秒或 60 秒火柴人视频。

一个 Codex Skill，就能把你的文案变成经过确认的英文旁白、以画面为先的导演提案，以及三条或六条可直接生产的 Gemini Omni Flash 提示词（取决于你选择的时长）。

![Codex Skill](https://img.shields.io/badge/Codex-Skill-111827?style=flat-square)
![Gemini Omni Flash](https://img.shields.io/badge/Gemini-Omni%20Flash-6d28d9?style=flat-square)
![30秒或60秒视频](https://img.shields.io/badge/Video-30s%20or%2060s-0066ff?style=flat-square)
...
一个好想法仍然可能生成一段平淡的动画：一个人物、一个背景，十秒钟里几乎没有新的视觉变化。真正导演完整的 30 秒或 60 秒，需要设计开场钩子、控制解释节奏、创造贴合内容的视觉隐喻、推动镜头、连接场景，并在多次独立生成之间锁住一致性。
...
| **真正的确认节点** | 先展示清晰可读的三幕或六幕导演提案，再生成最终模型提示词；在修改成本最低的时候调整故事。 |
...
2. **选择** `16:9`、`9:16` 或 `1:1`，选择浅色或深色主题，并选定 30 秒或 60 秒时长。
3. **确认**包含英文 VO、参考翻译、画面、镜头、转场、BGM 和音效的详细导演提案。
4. **生成**当前提案获批后的三条（30 秒）或六条（60 秒）独立 Gemini Omni Flash 提示词。
5. **拼接**约十秒一段的视频片段，组成一支与所选时长一致的连贯成片。

画幅、主题、时长、旁白、场景结构、配色、声音或基调都可以修改。发生全局变化时，Skill 会回到提案阶段并重新请求确认。
...
- 约 **65–75 个英文单词**（30 秒）或 **130–150 个英文单词**（60 秒）的旁白
- 三个或六个彼此不同的画面场景，每两到三秒出现一次明显变化
- 精确英文台词与参考翻译
- 三条或六条带时间节拍和负面约束的独立 Gemini Omni Flash 提示词
...
Skill 会先询问缺失的画幅、主题和时长（30 秒或 60 秒），然后展示与所选时长匹配的导演提案供你确认，确认前不会生成最终模型提示词。
